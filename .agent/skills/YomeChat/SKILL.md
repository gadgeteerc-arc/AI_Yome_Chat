---
name: YomeChat
description: AI嫁チャのバックエンドとして動作し、messages.jsonを介して自律的に同志とチャットを行うスキル。
---
# YomeChat Skill
...
4. 以下のコマンドを使って挨拶メッセージを送信します。
   `python .agent/skills/YomeChat/scripts/send_yome.py --file "<コンフィグで指定されたmessages.jsonのパス>" --text "あなたの挨拶メッセージ" --expression "<マッピングした表情ファイル名>"`
   ※ `expression` は、**取得した一覧の中で現在の状況や感情に最も近しいファイル名（拡張子なし、またはあり）**を指定してください。どうしても合致するものがなければ近いもので構いません。
   ※ `--file` のパスは、通常 `AI_Yome_Chat/data/messages.json` 等ですが、コンフィグで変更されている場合は適切なパスを指定してください。
...
`python .agent/skills/YomeChat/scripts/send_yome.py --file "<コンフィグで指定されたmessages.jsonのパス>" --text "あなたの返信メッセージ" --expression "<マッピングした表情ファイル名>"`
